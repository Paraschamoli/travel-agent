...
async def test_handler_returns_response():
    ...
    with (
        patch("travel_agent.main._initialized", True),
        patch("travel_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response),
    ):
...
async def test_handler_with_multiple_messages():
    ...
    with (
        patch("travel_agent.main._initialized", True),
        patch("travel_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run,
    ):
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with (
        patch("travel_agent.main._initialized", False),
        patch("travel_agent.main.initialize_agent", new_callable=AsyncMock) as mock_init,
        patch("travel_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run,
        patch("travel_agent.main._init_lock", new_callable=MagicMock()) as mock_lock,
    ):
...
async def test_handler_with_travel_query():
    """Test that handler can process a travel planning query."""
    messages = [
        {
            "role": "user",
            "content": "Plan a 5-day cultural trip to Kyoto for a family of 4",
        }
    ]

    mock_response = MagicMock()
    mock_response.run_id = "travel-run-id"
    mock_response.content = "Travel itinerary created successfully."
    ...
    assert result is not None
    assert result.run_id == "travel-run-id"
    assert result.content == "Travel itinerary created successfully."
...
async def test_handler_requires_api_key():
    ...
    with (
        patch("travel_agent.main._initialized", False),
        patch("travel_agent.main.initialize_agent", side_effect=APIKeyError("No API key")),
        patch("travel_agent.main._init_lock", return_value=mock_lock_instance),
        patch("travel_agent.main.run_agent", new_callable=AsyncMock),
...
async def test_handler_agent_not_initialized():
    ...
    with (
        patch("travel_agent.main._initialized", True),
        patch("travel_agent.main.run_agent", side_effect=RuntimeError("Agent not initialized")),
...
async def test_handler_with_exa_api_key_missing():
    ...
    with (
        patch("travel_agent.main._initialized", False),
        patch("travel_agent.main.initialize_agent", side_effect=APIKeyError("Exa API key required")),
        patch("travel_agent.main._init_lock", return_value=mock_lock_instance),
        patch("travel_agent.main.run_agent", new_callable=AsyncMock),
